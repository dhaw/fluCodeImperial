function [f,g,z2]=subPandemicSimulationVax(NNbar,params,xdata,plotComp,plotEpis,ydata,tswitch,vaxparams)
%{
Generate a single model simulation with arguments as follows:
NNbar - state populations by age
params - parameters with uncertainty
xdata - weeks of reported data
plotComp - =1 to plot comparison of simulation and data; =0 otherwise
plotEpis - =1 to plot ode output only; =0 otherwise
ydata - hospitalisations reported
tswitch - time of school openings (fall)
vaxparams - vaccination rates and effectiveness by month/age group
%}
...
%
age2mats=1;
%Flag used for 2 distinct age-mixung matrices
%Final manuscript requires mcmc=0, age2mats=1
%EVEN THOUGH MCMC IS USED
...
ph2=.0009;
t0shift=0;
tlag=-45;%Peak timing for seasonality
dMonth=[-inf,273,304,334,365,396,424,454,10^4];%Start points of relevant months
%%
...
if ages==4
    ...
    mu=zeros(4,1);
    tdays=30;%Days per month (denominator for aggregation)
    legString={'0-4','5-19','20-64','65+'};
    from57=13/45;
elseif ages==5
    nbar=length(NNbar);
    meanHosp=[393.0256  999.3054  702.6867  406.0680  177.1958];
    hosp=1./meanHosp';%Rate=1/timePeriod
    ...
    mu=zeros(5,1);
    tdays=7;%Days per week (denominator for aggregation)
    legString={'0-4','5-17','18-49','50-64','65+'};
    from57=24/30;
end
%
tswitchAttack=243;%Always secndary calculate attck rate from here
%%
%Plotting options:
logPlots=0;
...
relInc=0;%Relative incidence - fraction of age group population - f and g
%%
%Fixed parameters:
seednum=6;
closureFactor=cf;
...
if mcmc==0
    ...
    if age2mats==1
        %Only relevant for MLE fit on whole wave
...
%Calculate betas (Go=NGM with schools open):
...
Sdot=-Sfoi.*S-v1.*S;
SVHdot=-Sfoi.*SVH+v1.*S-SVH/14;%14 days for vaccine to take effect
